...
char *coidn;
   ...
   //
   volatileObjectCleanerThreadRunning = CreateThread(NULL,1024,
   																database::deleteVolatileObjects,
                                                   (LPVOID)this,NULL,
...
database::~database(void){
   ...
   if (volatileObjectCleanerThreadRunning!=FALSE)
   	if (TerminateThread(volatileObjectCleanerThreadRunning,1)==FALSE)
      	cout<<"Thread Termination failed";
      else cout<<"Thread Termination succeeded";
   else cout<<"Thread is not running";
...
DWORD database::deleteVolatileObjects(DWORD *d)
{
   ...
   while (1)
   {
   	...
   	while (o!=NULL)
   	{
   		if((db->getproperty_value((ODB_REF)o,(ODB_NAME)TIMETOLIVE,(ODB_INT *)&ttl)>0)&&(o->getAge()>ttl))
         {
          	cout<<" deleted one !";
         	db->delete_object(o);
         }
