...
#include "d:\users\staffan\odb\source\parser.h"
#include <stdio.h>
#include <string.h>
#include <iostream.h>
#include <stdlib.h>
...
ODBTokenType t,token;
   ...
   //-----------------------------------------------
   while((whiteChar(currentCh))&&(rv!=EOF)) rv=fscanf(inputfile,"%c",&currentCh);
   if (rv==EOF) return EOFTOKEN;
   ...
   if ((comment==0)&&(token==SLASHTOKEN)){
   	comment=1; 	//signals that a comment is currently investigated. Ie this
...
int i=0;
	while ((i<NOF_KEYWORDS)&&((keyWord[i]==NULL)||(strcmp(str,keyWord[i])!=0))) i++;
