...
char *bufp;
	...
	switch(type){
			...
			case _CHAR_:
				...
				break;
			case _OBJECT_:
				if (obj!=NULL) (*obj).displayoid(buf);
				break;
			case _COLLECTION_:
				(*setval).display(buf);
				break;
...
char charval[256],keyword[20],ch;
	keyword[0]='\0'; //initialized to other than }
	//The set may be empty scan until } or until some other charcater
	ch=(char)(*databasefile).peek();
	while ((isspace(ch)!=0)&&(ch!='}')){
		(*databasefile).get();  //remove the charcter
		ch=(char)(*databasefile).peek(); //peek the next charcter
		keyword[0]=ch;
		if (ch=='}') ch=(char)(*databasefile).get(); //read it since empty set
		}
	keyword[1]='\0';
	...
	switch(datatype){
		...
		case _CHAR_:
			while ((*databasefile).peek()!='}'){
				if ((*databasefile).peek()==' ') ch=(char)(*databasefile).get();//read ' '
				if ((*databasefile).peek()=='\n') ch=(char)(*databasefile).get(); //read \n
