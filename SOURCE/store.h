...
#define _NULL_ 		       	"NULL"
#define _OID_ 		       	"OID"
#define TIMETOLIVE 	       	"timeToLive"
#define DEFAULTTIMETOLIVE 		30
#define AUTO_DELETE_VOLATILES_INTERVAL 120
#define AUTOSAVE_INTERVAL		60
#define MUTEX_TIME_OUT        30000 // 30 seconds
...
public:
  ...
  int 	populatefromfile(ifstream *databasefile,int datatype,
			 struct tblentry *maptable,database *db);
};
...
public:
  ...
  int      getDataType(void){return valuetype;};
};
...
//--------------------------------------------------
#ifdef __DLL__
class _export volobject:public object
#else
class volobject:public object
#endif
{
protected:
  //struct time ctm;
...
protected:
  ...
  indextp  *populateindex(ODB_NAME nm,
			  indextp *thenewindex);
...
public:
  ...
  type *add_inheritedproperty(int tp, 
			      ODB_NAME nm);
  ...
  virtual ODB_REF create_instance(void);
  ...
  query_stream 	 *open(ODB_NAME nm,ODB_CHAR op,ODB_CHAR charstringval);
  ...
  virtual int 	  saveobjects(ofstream *databasefile,int how);
...
#ifdef __DLL__
class _export voltype : public type
#else
class voltype : public type
#endif
{
private:
  //struct time ctm;
...
public:
  voltype(void);
  voltype(ODB_NAME nm, database *db);
  voltype(ODB_NAME nm, type *super, database *db);
  ~voltype(void);
  int 	  saveobjects(ofstream *databasefile,int how);
...
{protected:
  ...
  int          eval_op(char *oop,ODB_CHAR chval,ODB_CHAR cmpval);
  ...
  // functions on data
  //
  int          subString(ODB_CHAR mains, ODB_CHAR subs);
...
public:
  ...
  select_stream    *open_select_stream(ODB_NAME prop,ODB_NAME oop,
				       ODB_INT intval);
  select_stream    *open_select_stream(ODB_NAME prop,ODB_NAME oop, 
				       ODB_REAL realval);
  ...
  semi_join_stream *open_sj_stream(ODB_NAME prop1, query_stream *st2, 
				   ODB_NAME prop2);
...
//--------------------------------------------------
#ifdef __DLL__
class _export select_stream : public query_stream
#else
...
public:
  ...
  select_stream(query_stream *q,ODB_NAME nm,ODB_NAME oop,ODB_CHAR chrstr);
...
//--------------------------------------------------
// Classsemi join strem
// Description	a stream that joins two streams
// Authorstaffan flodin
//--------------------------------------------------
#ifdef __DLL__
class _export semi_join_stream : public query_stream
#else
...
public:
  ...
  semi_join_stream(query_stream *lop,ODB_NAME lnm,query_stream *rop,
		   ODB_NAME rnm);
...
private:
  ...
  parser       ODBparser;
  DWORD	       volatileObjectCleanerThread;
  HANDLE       volatileObjectCleanerThreadRunning;
  DWORD	       databaseWriterThread;
  HANDLE       databaseWriterThreadRunning;
  char         filename[FILENAME_LENGTH];
  void         setFileName(char *fnm){strcpy(filename,fnm);};
  //
  // semaphores
  //
  HANDLE         updateDBrequest;
  char          updatDBrequestMutexName[32];
...
public:
  ...
  query_stream     *open_stream(ODB_NAME tpname, ODB_NAME propnm,ODB_NAME op,
				ODB_INT i);
  query_stream     *open_stream(ODB_NAME tpname, ODB_NAME propnm,ODB_NAME op,
				ODB_REAL f);
  query_stream     *open_stream(ODB_NAME tpname, ODB_NAME propnm,ODB_CHAR ch);
  query_stream     *open_stream(ODB_NAME tpname, ODB_NAME propnm,ODB_CHAR op,
				ODB_CHAR ch);
  ...
  select_stream    *select(query_stream *qst,ODB_NAME propnm,ODB_NAME oop,
			   ODB_INT i);
  select_stream    *select(query_stream *qst,ODB_NAME propnm,ODB_NAME oop,
			   ODB_REAL f);
  ...
  semi_join_stream *semi_join(query_stream *ls,ODB_NAME p1,query_stream *rs,
			      ODB_NAME p2);
  ...
  static DWORD WINAPI deleteVolatileObjects(database *db);
  static DWORD WINAPI databaseWriter(database *db);

  //
  // Synchonization stuff
  //
  char *getUpdatDBrequestMutexName(void) {return updatDBrequestMutexName;};
