...
BOOL DlgCreateInst::callbackfn(HWND h, UINT mess, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (mess)
	...
	case PB_SET:
	  ...
	  //
	  index=SendDlgItemMessage(h,CMB_CREINSTTYPE,CB_GETCURSEL,0,0); 
	  SendDlgItemMessage(h,CMB_CREINSTTYPE,CB_GETLBTEXT,index,
			     (LPARAM)tpnm);
	  ...
	  if ((strlen(newvalue)>0)&&(strlen(propname)>0)&&(oktype!=0))
	    {
	      ...
	      SendDlgItemMessage(h,LB_ASSPROP,LB_ADDSTRING,noassignedprops,
				 (LPARAM)propname);
	      SendDlgItemMessage(h,LB_PROPVALS,LB_ADDSTRING,noassignedprops,
				 (LPARAM)newvalue);
